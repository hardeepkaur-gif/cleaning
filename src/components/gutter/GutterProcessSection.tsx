...
import local from "./GutterProcessSection.module.css";
import {
  gutterProcessHubLabel,
  gutterProcessIntro,
  gutterProcessSteps,
  gutterProcessTitle,
  type GutterProcessStep,
} from "./gutterProcessData";
  ...
  return (
    ...
    >
      <div className={styles.cardIconWrap} ref={iconRef}>
        ...
        <span className={styles.cardIcon} role="img" aria-label={step.iconAlt}>
          <Icon aria-hidden />
        </span>
      </div>
...
export default function GutterProcessSection() {
  ...
  const rowCount = Math.max(leftSteps.length, rightSteps.length);
    ...
    >
      <div className={styles.container}>
        <div className={`${styles.topHeader} ${local.topHeader}`}>
          <h2 className={styles.title} id="gutter-process-title">
            {gutterProcessTitle}
          </h2>
          <p className={local.intro}>{gutterProcessIntro}</p>
        </div>
        ...
        <div ref={radialRef} className={styles.desktopRadial}>
          ...
          <div ref={hubRef} className={styles.hub}>
            ...
            <div className={styles.hubCore}>
              <strong>{gutterProcessHubLabel}</strong>
            </div>
          </div>
        ...
        <div ref={mobileFlowRef} className={styles.mobileFlow}>
          ...
          {gutterProcessSteps.map((step, index) => (
            <StepCard
              key={step.title}
              step={step}
              index={index}
